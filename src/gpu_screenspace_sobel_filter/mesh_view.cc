 ...
 *
 * Modified for contour rendering with sobel shader by Jeroen Baert
 * www.forceflow.be
 */
...
// Shader programs
// Sobel
GLhandleARB shader_sobel;
GLhandleARB shader_sobel_vert;
GLhandleARB shader_sobel_frag;
...
void setup_uniforms()
{
	glUniform1fARB(loc_limit, LIMIT);
...
void redraw()
{
	...
	glDisable(GL_LIGHTING);
	draw_mesh(0);
	// We don't use FBO, since support is limited
    ...
    glBindTexture(GL_TEXTURE_2D, depth_texture);
    // enable shader
    glUseProgramObjectARB(shader_sobel);
	setup_uniforms();
...
{
	// load Sobel shader
	load_shader(shader_sobel,shader_sobel_vert,shader_sobel_frag,"sobel.vert","sobel.frag");
	// bind our edge texture to it
	glUseProgramObjectARB(shader_sobel);
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, depth_texture);
	GLint handle = glGetUniformLocationARB(shader_sobel, "edge");
	loc_limit = glGetUniformLocationARB(shader_sobel, "limit");
	loc_width = glGetUniformLocationARB(shader_sobel, "width");
	glUniform1fARB(handle, 0);
	glUseProgramObjectARB(0);
}
...
void setup_textures()
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, WIDTH, WIDTH, 0, GL_DEPTH_COMPONENT,GL_FLOAT, 0);
}
...
int main(int argc, char *argv[])
{
	...
	glui_window->add_slider("LIMIT", GLUI_SLIDER_FLOAT, 0, 2, &LIMIT);
